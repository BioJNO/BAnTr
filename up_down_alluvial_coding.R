# expr.dat <- read.csv("expression_data_update.csv")
...
all_sig <- subset(coding,
                  A.LEP.vs.PRE.Pval < 0.01 & A.LEP.vs.PRE.log2FC > 1 |
                    A.LEP.vs.PRE.Pval < 0.01 & A.LEP.vs.PRE.log2FC < -1 |
                    A.PAC.vs.A.LEP.Pval < 0.01 & A.PAC.vs.A.LEP.log2FC > 1 |
                    A.PAC.vs.A.LEP.Pval < 0.01 & A.PAC.vs.A.LEP.log2FC < -1 |
                    TET.vs.A.PAC.Pval < 0.01 & TET.vs.A.PAC.log2FC > 1 |
                    TET.vs.A.PAC.Pval < 0.01 & TET.vs.A.PAC.log2FC < -1)
...
alluvial_plot <- ggplot(plot.data,
  ...
  geom_flow(stat = "alluvium", lode.guidance = "frontback", alpha=0.1) +
...
# ggsave("Differential.Expression.Anther.only.svg", dpi=600)
# ggsave("Differential.Expression.Anther.only.tiff", dpi=600)


alluvial_plot

ggsave("AntherDifferentialExpressionCodingOnly.svg", dpi=600)
